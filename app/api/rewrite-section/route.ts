...
export async function POST(req: NextRequest) {
  ...
  try {
    switch (section.kind) {
      case 'summary': {
        const out = await callAI(summaryPrompt(formData.rawSummary, jobTitle, keywords), { userId, maxRetries: 3, maxTokens: 800 })
        return NextResponse.json({ summary: out })
      }

      case 'skills': {
        const raw = await callAI(skillsPrompt(formData.rawSkills, jobTitle, keywords), { userId, maxRetries: 3, maxTokens: 800 })
      ...
      case 'experience': {
        ...
        const raw = await callAI(experiencePrompt(exp, jobTitle, keywords), { userId, maxRetries: 3, maxTokens: 1000 })
        const bullets = await safeParseJSONWithHeal<string[]>(raw, [exp.rawDuties], userId)
      ...
      case 'project': {
        ...
        const raw = await callAI(projectPrompt(proj, jobTitle), { userId, maxRetries: 3, maxTokens: 600 })
        const bullets = await safeParseJSONWithHeal<string[]>(raw, [proj.description], userId)
        return NextResponse.json({ bullets, index: section.index })
      }
    ...
    if (status === 402) {
      const msg = err instanceof Error ? err.message : 'Out of credits.'
